...
const recuperarLimiter = rateLimit({
  ...
  message: {
    ok: false,
    message:
      "Demasiadas solicitudes de recuperación. Espera una hora e intenta de nuevo.",
  },
});
...
const registerLimiter = rateLimit({
  ...
  message: {
    ok: false,
    message:
      "Demasiadas solicitudes de registro. Espera una hora e intenta de nuevo.",
  },
});
...
if (!process.env.JWT_SECRET || !process.env.JWT_REFRESH_SECRET) {
  console.error(
    "❌ FATAL: JWT_SECRET o JWT_REFRESH_SECRET no están definidos en las variables de entorno.",
  );
  console.error(
    "   El servidor no puede arrancar de forma segura sin estas claves.",
  );
  process.exit(1);
}
...
app.get("/api/health", (req, res) => {
  res.json({
    ok: true,
    message: "API Parksmart funcionando",
    timestamp: new Date(),
  });
});
