  ...
  try {
    ...
    return res
      ...
      .json({
        ok: false,
        message:
          "No se pudieron cargar los centros de formación. Estamos trabajando en ello.",
      });
  }
});
  ...
  try {
    ...
    return res
      ...
      .json({
        ok: false,
        message:
          "No se pudieron cargar los centros asignados. Estamos trabajando en ello.",
      });
  }
});
  ...
  try {
    ...
    return res
      ...
      .json({
        ok: false,
        message:
          "No se pudieron cargar las regiones. Estamos trabajando en ello.",
      });
  }
});
...
router.get("/config/:id_centro", async (req, res) => {
  ...
  if (!id_centro)
    return res.status(400).json({ ok: false, message: "id_centro inválido." });
  ...
  try {
    ...
    return res
      ...
      .json({
        ok: false,
        message:
          "No se pudo cargar la configuración del parqueadero. Estamos trabajando en ello.",
      });
  }
});
...
router.post(
  ...
  [
    ...
    body("tipos")
      .isArray({ min: 1 })
      .withMessage("Selecciona al menos un tipo de vehículo."),
  ],
  async (req, res) => {
    ...
    if (!id_centro)
      return res
        .status(400)
        .json({ ok: false, message: "id_centro inválido." });
    ...
    if (modo === "controlado" && (!capacidad || Number(capacidad) < 1)) {
      return res
        ...
        .json({
          ok: false,
          message: "El modo controlado requiere una capacidad mayor a 0.",
        });
    }
    ...
    try {
      ...
      const centroCheck = await client.query(
        "SELECT id_centro FROM centros_formacion WHERE id_centro = $1",
        [id_centro],
      );
      if (!centroCheck.rows.length) {
        ...
        return res
          .status(404)
          .json({ ok: false, message: "Centro de formación no encontrado." });
      }
      ...
      if (!tiposR.rows.length) {
        ...
        return res
          .status(400)
          .json({ ok: false, message: "Ningún tipo de vehículo reconocido." });
      }
      ...
      return res.status(500).json({
        ok: false,
        message:
          "No se pudo crear el lado del parqueadero. Estamos trabajando en ello.",
      });
    } finally {
      client.release();
    }
...
router.put("/config/:id_centro/lados/:id_lado/toggle", async (req, res) => {
  ...
  try {
    ...
    if (!check.rows.length)
      return res
        .status(404)
        .json({ ok: false, message: "Lado no encontrado para este centro." });

    const nuevoEstado = !check.rows[0].habilitado;
    await query(`UPDATE lados SET habilitado = @estado WHERE id_lado = @id`, {
      estado: nuevoEstado,
      id: id_lado,
    });
    ...
    return res
      ...
      .json({
        ok: false,
        message:
          "No se pudo cambiar el estado del lado del parqueadero. Estamos trabajando en ello.",
      });
  }
});
...
router.put(
  "/config/:id_centro/lados/:id_lado",
  [
    body("nombre").trim().notEmpty().withMessage("Nombre requerido."),
    body("modo").isIn(["controlado", "libre"]).withMessage("Modo inválido."),
    body("capacidad").optional({ nullable: true }).isInt({ min: 1, max: 9999 }),
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!tiposR.rows.length) {
        ...
        return res
          ...
          .json({
            ok: false,
            message:
              "Ningún tipo de vehículo reconocido. Selecciona al menos uno.",
          });
      }

      await client.query(
        `DELETE FROM lados_tipos_permitidos WHERE id_lado = $1`,
        [id_lado],
      );
      ...
      return res.json({
        ok: true,
        message: `Lado "${nombre}" actualizado.`,
        data: { id_centro, lados },
      });
    } catch (err) {
      await client.query("ROLLBACK");
      ...
      return res
        ...
        .json({
          ok: false,
          message:
            "No se pudo actualizar la configuración del lado. Estamos trabajando en ello.",
        });
    } finally {
      client.release();
    }
...
router.delete("/config/:id_centro/lados/:id_lado", async (req, res) => {
  ...
  try {
    ...
    await client.query(
      `DELETE FROM registros_uso          WHERE id_lado = $1`,
      [id_lado],
    );
    await client.query(
      `DELETE FROM lados_tipos_permitidos WHERE id_lado = $1`,
    ...
    await client.query(
      `DELETE FROM cupos                  WHERE id_lado = $1`,
      [id_lado],
    );
    await client.query(
      `DELETE FROM lados                  WHERE id_lado = $1`,
      [id_lado],
    );
    ...
    return res
      ...
      .json({
        ok: false,
        message:
          "No se pudo eliminar el lado del parqueadero. Estamos trabajando en ello.",
      });
  } finally {
    client.release();
  }
...
router.post(
  "/config/centros-admin",
  [
    body("nombre")
      .trim()
      .notEmpty()
      .withMessage("El nombre del centro es requerido."),
    body("id_region")
      .isInt({ min: 1 })
      .withMessage("Selecciona una región válida."),
  ],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
    ...
    try {
      ...
      if (!regionCheck.rows.length)
        return res
          .status(404)
          .json({ ok: false, message: "La región seleccionada no existe." });
      ...
      if (dupCheck.rows.length)
        return res
          ...
          .json({
            ok: false,
            message: "Ya existe un centro con ese nombre en esa región.",
          });
      ...
      return res
        ...
        .json({
          ok: false,
          message:
            "No se pudo crear el centro de formación. Estamos trabajando en ello.",
        });
    }
  },
);
...
router.put(
  "/config/centros-admin/:id_centro",
  [
    body("nombre")
      .trim()
      .notEmpty()
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!centroCheck.rows.length)
        return res
          .status(404)
          .json({ ok: false, message: "Centro no encontrado." });
      ...
      if (dupCheck.rows.length)
        return res
          ...
          .json({
            ok: false,
            message: "Ya existe otro centro con ese nombre en esa región.",
          });
      ...
      return res
        ...
        .json({
          ok: false,
          message:
            "No se pudo actualizar el centro de formación. Estamos trabajando en ello.",
        });
    }
  },
);
...
router.delete("/config/centros-admin/:id_centro", async (req, res) => {
  ...
  try {
    ...
    if (idLados.length > 0) {
      ...
      await client.query(`DELETE FROM registros_uso WHERE id_lado = ANY($1)`, [
        idLados,
      ]);
      ...
      await client.query(`DELETE FROM cupos WHERE id_lado = ANY($1)`, [
        idLados,
      ]);
      await client.query(`DELETE FROM lados WHERE id_centro = $1`, [id_centro]);
    }

    // Finalmente eliminar el centro
    await client.query(`DELETE FROM centros_formacion WHERE id_centro = $1`, [
      id_centro,
    ]);
    ...
    return res.json({
      ok: true,
      message: `Centro "${nombre}" eliminado correctamente.`,
    });
  } catch (err) {
    await client.query("ROLLBACK");
    ...
    return res
      ...
      .json({
        ok: false,
        message:
          "No se pudo eliminar el centro de formación. Estamos trabajando en ello.",
      });
  } finally {
    client.release();
  }
