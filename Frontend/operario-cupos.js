...
function refreshLadoA() {
  ...
  if (circle)
    circle.className =
      "cupo-circle " + (pct >= 80 ? "danger" : pct >= 50 ? "warn" : "ok");
  renderAGrid();
...
function adjustA(delta) {
  ...
  refreshLadoA();
  showToast(
    delta > 0 ? "Espacio ocupado (Lado A)" : "Espacio liberado (Lado A)",
  );
}
...
function refreshLadoB() {
  ...
  const totalHoy =
    ladoB.carros.entradas + ladoB.motos.entradas + ladoB.bicis.entradas;
  const setEl = (id, v) => {
...
function updateCuposUI() {
  ...
  if (barA) {
    ...
    barA.querySelector("span").textContent = pctA + "%";
  }
