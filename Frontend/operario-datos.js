...
async function cargarCatalogos() {
  try {
    ...
    console.warn("Error catálogos:", e);
  }
}
...
async function cargarPerfilAdmin() {
  try {
    ...
    if (data.ok && data.data) {
      ...
      if (u.email) {
        const el = document.getElementById("a-email");
      ...
      if (u.foto_perfil && imgEl) {
        ...
        if (tbAv) {
          tbAv.style.backgroundImage = `url(${u.foto_perfil})`;
    ...
    console.warn("Error perfil admin:", e);
  }
}
...
async function cargarCuposDesdeAPI() {
  try {
    ...
    const [dataOcup, dataRol] = await Promise.all([
      resOcup.json(),
      resRol.json(),
    ]);
    ...
    if (dataOcup.ok && dataOcup.data) {
      dataOcup.data.forEach((row) => {
        const ladoNombre = (row.lado || row.Lado || "")
          .toString()
          .toUpperCase()
          .trim();
...
async function cargarRecientesDesdeAPI() {
  ...
  const emptyRow = (cols) =>
    `<tr><td colspan="${cols}" style="text-align:center;padding:20px;opacity:0.5">Sin actividad reciente hoy</td></tr>`;
  const errorRow = (cols, msg) =>
    `<tr><td colspan="${cols}" style="text-align:center;padding:16px;color:#ef9a9a;font-size:13px;">${msg}</td></tr>`;
  ...
  try {
    ...
    if (!res) {
      if (tbM)
        tbM.innerHTML = errorRow(6, "Sesión expirada. Recarga la página.");
    ...
    if (!res.ok) {
      ...
      console.error(
        "cargarRecientesDesdeAPI HTTP error:",
        res.status,
        res.statusText,
      );
    ...
    if (!data.ok) {
      ...
      if (tbM)
        tbM.innerHTML = errorRow(
          6,
          data.message || "Error al cargar actividad",
        );
    ...
    registros.forEach((r) => {
      try {
        ...
        const diaReg = fechaAcc
          ? fechaAcc.toLocaleDateString("en-CA", { timeZone: "America/Bogota" })
          : null;
        const esHoy = diaReg === hoyStr;
        const time = fechaAcc
          ? esHoy
            ? fechaAcc.toLocaleTimeString("es-CO", {
                hour: "2-digit",
                minute: "2-digit",
                timeZone: "America/Bogota",
              })
            : fechaAcc.toLocaleDateString("es-CO", {
                day: "2-digit",
                month: "short",
                timeZone: "America/Bogota",
              }) +
...
function renderChartHora(porHora = []) {
  if (chartHoraInst) {
    chartHoraInst.destroy();
    chartHoraInst = null;
  }
  const wrap = document.querySelector("#chartHora")?.parentElement;
  const total = porHora.reduce(
    (s, r) => s + Number(r.entradas || 0) + Number(r.salidas || 0),
    0,
  );
  if (!total) {
    if (wrap)
      wrap.innerHTML =
        '<div class="chart-no-data"><i class="bi bi-bar-chart-line"></i>Sin actividad registrada hoy</div>';
    return;
  }
      ...
      datasets: [
        {
          label: "Entradas",
          data: entradas,
          backgroundColor: "rgba(66,165,245,0.90)",
...
function renderChartSemana(porSemana = []) {
  if (chartSemanaInst) {
    chartSemanaInst.destroy();
    chartSemanaInst = null;
  }
  ...
  if (!total) {
    if (wrap)
      wrap.innerHTML =
        '<div class="chart-no-data"><i class="bi bi-graph-up"></i>Sin ingresos en los últimos 7 días</div>';
    return;
  }
  if (wrap && !wrap.querySelector("canvas")) {
...
function renderUsersTable(list) {
  ...
  if (!list.length) {
    tbody.innerHTML =
      '<tr><td colspan="8" style="text-align:center;color:rgba(255,255,255,0.4);padding:24px">Sin resultados</td></tr>';
    return;
  }
  tbody.innerHTML = list
    .map((u) => {
      ...
      if (dentro) {
        ...
        const ladoOpts =
          '<option value="1">Lado A — Espacios controlados</option><option value="2">Lado B — Espacio abierto</option>';

        let vehiculoOpts = "";
        if (u.vehiculos && u.vehiculos.length > 1) {
          vehiculoOpts =
            `<select class="inline-select" id="veh-sel-${u.id}" style="max-width:80px; margin-right:4px;">` +
            u.vehiculos
              .map((v) => {
      ...
      return `
      <tr>
        <td><div class="user-cell"><div class="mini-av">${
          u.foto
            ? `<img src="${u.foto}" alt="${u.nombre}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`
            : u.nombre
                .split(" ")
                .map((w) => w[0])
                .slice(0, 2)
                .join("")
        }</div><button class="user-name-link" onclick="showUserCard('${u.id}')" title="Ver carnet de ${u.nombre}">${u.nombre}</button></div></td>
...
function filterUsers() {
  ...
  const filtered = usuarios.filter((u) => {
    ...
    const matchV =
      !vf ||
      (u.vehiculos && u.vehiculos.some((v) => v.tipo.toLowerCase() === vf));
...
async function toggleUserStatus(id) {
  ...
  if (exitBtn) {
    ...
    exitBtn.innerHTML = '<i class="bi bi-hourglass-split"></i>';
  }
  try {
    ...
    if (!data.ok) {
      ...
      if (exitBtn) {
        ...
        exitBtn.innerHTML = '<i class="bi bi-arrow-up-circle-fill"></i> Salida';
      }
      return;
    }
  } catch (e) {
...
async function toggleUserStatusWithLado(id) {
  ...
  if (entBtn) {
    ...
    entBtn.innerHTML = '<i class="bi bi-hourglass-split"></i>';
  }
  ...
  if (!id_vehiculo) {
    ...
    if (entBtn) {
      ...
      entBtn.innerHTML = '<i class="bi bi-arrow-down-circle-fill"></i> Entrada';
    }
    return;
  }
...
let _chartHoraAInst = null,
  _chartTipoAInst = null;
let _chartHoraBInst = null,
  _chartSemanaBInst = null,
  _chartTipoBInst = null;

function switchDashLado(lado, btn) {
  document
    .querySelectorAll(".dash-lado-tab")
    .forEach((b) => b.classList.remove("active"));
  btn.classList.add("active");
  document
    .querySelectorAll(".dash-side-panel")
    .forEach((p) => (p.style.display = "none"));
  const panelId =
    lado === "global" ? "dash-panel-global" : "dash-panel-" + lado;
...
async function cargarStatsLado(id_lado) {
  try {
    ...
    console.warn("Error stats-lado:", e);
  }
}
...
function renderChartHoraA(porHora = []) {
  if (_chartHoraAInst) {
    _chartHoraAInst.destroy();
    _chartHoraAInst = null;
  }
  const total = porHora.reduce(
    (s, r) => s + Number(r.entradas || 0) + Number(r.salidas || 0),
    0,
  ...
  if (!total) {
    if (wrap)
      wrap.innerHTML =
        '<div class="chart-no-data"><i class="bi bi-bar-chart-line"></i>Sin actividad en Lado A hoy</div>';
    return;
  }
      ...
      datasets: [
        {
          label: "Entradas A",
          data: entradas,
          backgroundColor: "rgba(99,179,237,0.92)",
        ...
        },
        {
          label: "Salidas A",
          data: salidas,
          backgroundColor: "rgba(252,129,74,0.88)",
...
function renderChartTipoA(porTipo = []) {
  ...
  const valores = tipoMap.map(({ match }) =>
    porTipo
      .filter((r) => r.tipo && match.includes(r.tipo.toLowerCase()))
  ...
  if (!total) {
    if (wrap)
      wrap.innerHTML =
        '<div class="chart-no-data"><i class="bi bi-pie-chart"></i>Sin registros de tipos hoy</div>';
    return;
  }
  const ctx = _rebuildCanvas("chartTipoA");
  if (!ctx) return;
  const colors = ["rgba(99,179,237,0.85)", "rgba(72,187,120,0.85)"];
  _chartTipoAInst = new Chart(ctx, {
    type: "doughnut",
    data: {
      labels: tipoMap.map((t) => t.label),
      datasets: [
        {
          data: valores,
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (c) =>
              ` ${c.label}: ${c.raw} (${total > 0 ? Math.round((c.raw * 100) / total) : 0}%)`,
          },
        },
      },
    },
...
function renderChartHoraB(porHora = []) {
  if (_chartHoraBInst) {
    _chartHoraBInst.destroy();
    _chartHoraBInst = null;
  }
  const total = porHora.reduce(
    (s, r) => s + Number(r.entradas || 0) + Number(r.salidas || 0),
    0,
  ...
  if (!total) {
    if (wrap)
      wrap.innerHTML =
        '<div class="chart-no-data"><i class="bi bi-bar-chart-line"></i>Sin actividad en Lado B hoy</div>';
    return;
  }
      ...
      datasets: [
        {
          label: "Entradas B",
          data: entradas,
          backgroundColor: "rgba(159,122,234,0.92)",
        ...
        },
        {
          label: "Salidas B",
          data: salidas,
          backgroundColor: "rgba(237,137,54,0.88)",
...
function renderChartSemanaB(porSemana = []) {
  if (_chartSemanaBInst) {
    _chartSemanaBInst.destroy();
    _chartSemanaBInst = null;
  }
  const total = porSemana.reduce((s, r) => s + Number(r.ingresos || 0), 0);
  ...
  if (!total) {
    if (wrap)
      wrap.innerHTML =
        '<div class="chart-no-data"><i class="bi bi-graph-up"></i>Sin ingresos en los últimos 7 días (Lado B)</div>';
    return;
  }
  ...
  _chartSemanaBInst = new Chart(ctx, {
    type: "line",
    data: {
      labels,
      datasets: [
        {
          label: "Ingresos Lado B",
          data: ingresos,
          borderColor: "rgba(159,122,234,1)",
          backgroundColor: "rgba(159,122,234,0.35)",
          tension: 0.4,
          fill: true,
          pointBackgroundColor: "#fff",
          pointBorderColor: "rgba(159,122,234,1)",
          pointBorderWidth: 2,
          pointRadius: 5,
          pointHoverRadius: 7,
        },
...
function showUserCard(qrId) {
  ...
  document.getElementById("uc-role-badge").innerHTML =
    `<i class="bi ${rolIcons[rol] || "bi-person-fill"}"></i> ${rolLabels[rol] || rol}`;
  ...
  if (u.vehiculos && u.vehiculos.length > 0) {
    ...
    // Solo conectar eventos si hay más de 1
    if (total > 1) {
  ...
  document.getElementById("uc-status-text").textContent = dentro
    ? "Dentro"
    : "Fuera";
  document.getElementById("uc-last-seen").textContent = dentro
    ? "Actualmente en el parqueadero"
    : "";
