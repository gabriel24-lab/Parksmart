...
function startScan() {
  ...
  if (status)
    status.innerHTML =
      '<i class="bi bi-camera-video-fill" style="color:#4fc3f7"></i> Iniciando cámara...';
  ...
  html5QrcodeScanner
    ...
    .then(() => {
      if (status)
        status.innerHTML =
          '<i class="bi bi-camera-video-fill" style="color:#69f0ae"></i> Cámara activa — apunta al QR';
    })
    .catch((err) => {
      ...
      if (currentFacingMode === "environment") {
        ...
        if (status)
          status.innerHTML =
            '<i class="bi bi-camera-video-off" style="color:#ef9a9a"></i> Sin acceso a cámara';
        showToast(
          "Otorga permisos de cámara en tu navegador o sube la imagen.",
          "error",
        );
...
function stopScan() {
  ...
  if (status)
    status.innerHTML = '<i class="bi bi-camera-video-off"></i> Cámara inactiva';
}
...
function showScanResult(data) {
  ...
  const initials = usuario.nombre_completo
    ...
    .toUpperCase();
  const idMask =
    "*".repeat(Math.max(0, String(usuario.numero_id).length - 4)) +
    String(usuario.numero_id).slice(-4);
  ...
  const estadoHTML = `
      <div class="scan-status-row">
        ...
        ${
          estado_actual
            ? `<span class="scan-status-extra">· ${estado_actual.lado} · desde ${fechaColombia(
                estado_actual.fecha_entrada,
...
function buildFotoHTML(veh) {
  ...
  const label = [veh.tipo, veh.placa || veh.modelo || veh.color]
    .filter(Boolean)
    .join(" ");
...
function buildActionButtons(usuario, vehiculos, dentro) {
  ...
  setTimeout(async () => {
    ...
    if (!lados.length) {
      ...
      const btnEnt = selLado
        .closest(".scan-user-body")
        ?.querySelector(".btn-entry");
      if (btnEnt) {
        btnEnt.disabled = true;
        btnEnt.title = "No hay lados habilitados";
      }
      return;
    }
    selLado.innerHTML = lados
      .map(
        (l) =>
...
async function adminEntrada(id_usuario, uid_key) {
  ...
  const id_vehiculo = (
    document.getElementById("sel-vehiculo-" + key) ||
    document.getElementById("sel-vehiculo")
  )?.value;
  const id_lado = (
    document.getElementById("sel-lado-" + key) ||
    document.getElementById("sel-lado")
  )?.value;
...
function registerEvent(userId, tipo) {
  ...
  if (u) {
    if (tipo === "entrada") {
      u.estado = "Dentro";
      cuposOcupados = Math.min(cuposOcupados + 1, cuposTotal);
    } else {
      u.estado = "Fuera";
      cuposOcupados = Math.max(cuposOcupados - 1, 0);
    }
    updateCuposUI();
...
function addRecentActivity(u, tipo, lado) {
  ...
  const vtag =
    u.vehiculo === "Carro" ? "car" : u.vehiculo === "Moto" ? "moto" : "bike";
...
function setRegVehicle(type, btn) {
  regVehicleType = type;
  document
    .querySelectorAll(".vehicle-tabs .vtab")
  ...
  if (entradaWrap)
    entradaWrap.style.display = type !== "ninguno" ? "block" : "none";
  if (type === "ninguno") {
...
async function registrarUsuario() {
  ...
  if (!nombre || !tipoId || !numId) {
    showToast(
      "Completa nombre, tipo de ID y número de identificación.",
      "error",
    );
    return;
  }
  ...
  try {
    ...
    showToast(
      `✓ Usuario "${nombre}" creado. Contraseña temporal: ${numId}`,
      "success",
    );
    ...
    if (regVehicleType !== "ninguno" && id_usuario) {
      if (btn)
        btn.innerHTML =
          '<i class="bi bi-hourglass-split"></i> Registrando vehículo...';
      ...
      if (regVehicleType === "bicicleta") {
        ...
        if (!modelo || !color) {
          showToast("Para la bicicleta completa modelo y color.", "error");
          if (btn) {
            btn.innerHTML =
              '<i class="bi bi-person-check-fill"></i> Registrar usuario';
            btn.disabled = false;
        ...
        const placa = document
          .getElementById(`rv-placa-${suffix}`)
          .value.trim();
        const color = document
          .getElementById(`rv-color-${suffix}`)
          .value.trim();
        const desc = document.getElementById(`rv-desc-${suffix}`).value.trim();
        if (!placa || !color) {
          showToast(
            `Para el ${regVehicleType} completa placa y color.`,
            "error",
          );
          if (btn) {
            btn.innerHTML =
      ...
      if (!dataV.ok) {
        showToast(
          "Usuario creado pero error al registrar vehículo: " +
            (dataV.message || ""),
          "error",
        );
      } else {
        ...
        setTimeout(
          () =>
            showToast(`✓ Vehículo registrado (ID: ${id_vehiculo})`, "success"),
          800,
        );
      }
    }
    ...
    if (conEntrada && id_vehiculo && id_usuario) {
      if (btn)
        btn.innerHTML =
          '<i class="bi bi-hourglass-split"></i> Registrando entrada...';
      const id_lado = parseInt(
        document.getElementById("reg-entrada-lado").value || "1",
      );
      ...
      if (!dataE.ok) {
        setTimeout(
          () =>
            showToast(
              "Vehículo registrado pero error en la entrada: " +
                (dataE.message || ""),
              "error",
            ),
          1600,
  ...
  if (btn) {
    btn.innerHTML = '<i class="bi bi-person-check-fill"></i> Registrar usuario';
    btn.disabled = false;
  }
}
...
function filterCentrosReg() {
  fillCentroSelect("reg-region", "reg-centro");
}
function filterCentrosAdmin() {
  fillCentroSelect("a-region", "a-centro");
}
  ...
  PhotoCrop.open(file, {
    onConfirm: async (blob) => {
      ...
      try {
        ...
        if (tbAv) {
          tbAv.style.backgroundImage = `url(${data.foto_url})`;
...
async function changeAdminPassword() {
  const btn = document.querySelector(
    "#section-perfil .glass-panel:last-child .btn-save",
  );
...
function setAdminVehicle(type, btn) {
  document
    .querySelectorAll("#section-vehiculo .vtab")
    .forEach((t) => t.classList.remove("active"));
  btn.classList.add("active");
  ["carro", "moto", "bicicleta"].forEach((t) => {
    document.getElementById("av-form-" + t).style.display =
      t === type ? "block" : "none";
  });
}
...
async function saveAdminVehicle(tipo) {
  ...
  const idTipo =
    tipo === "carro" ? 3 : tipo === "moto" ? 2 : tipo === "furgoneta" ? 4 : 1;
...
async function cargarDia(tipo) {
  ...
  if (tipo === "ayer") {
    fecha = ayer;
    document.getElementById("hist-admin-fecha").value = ayer;
  }
  ...
  const fLabel = new Date(fecha + "T12:00:00").toLocaleDateString("es-CO", {
    weekday: "long",
    year: "numeric",
    month: "long",
    day: "numeric",
  ...
  label.innerHTML =
    `<i class="bi bi-calendar-event" style="color:var(--brand-color);margin-right:5px;"></i>${fLabel}` +
    (esHoy
      ? ' <span style="background:rgba(76,175,80,0.2);color:#a5d6a7;font-size:11px;padding:2px 8px;border-radius:20px;margin-left:6px;">Hoy</span>'
      : "") +
    (esAyer
      ? ' <span style="background:rgba(255,193,7,0.15);color:#ffe082;font-size:11px;padding:2px 8px;border-radius:20px;margin-left:6px;">Ayer</span>'
      : "");
  ...
  try {
    ...
    if (
      ...
      for (let i = 1; i <= 60 && !encontrado; i++) {
        ...
        const prevFecha = d.toLocaleDateString("en-CA", {
          timeZone: "America/Bogota",
        });
        const prevData = await apiGet(
          `/parqueadero/historial-admin?fecha=${prevFecha}`,
        );
        if (prevData.ok && prevData.data && prevData.data.length > 0) {
          ...
          const fLabelPrev = new Date(fecha + "T12:00:00").toLocaleDateString(
            "es-CO",
            { weekday: "long", year: "numeric", month: "long", day: "numeric" },
          );
          label.innerHTML =
            `<i class="bi bi-calendar-event" style="color:var(--brand-color);margin-right:5px;"></i>${fLabelPrev}` +
...
function calcularResumenAdmin() {
  ...
  const promMin = compl.length
    ? Math.round(compl.reduce((s, r) => s + r.duracion_min, 0) / compl.length)
    : 0;
  const promH = Math.floor(promMin / 60);
  const promM = promMin % 60;

  const freq = {};
  haRegistros.forEach((r) => {
    freq[r.tipo_vehiculo] = (freq[r.tipo_vehiculo] || 0) + 1;
  });
  ...
  document.getElementById("ha-promedio").textContent =
    promMin > 0 ? formatDuracion(promMin) : "—";
  document.getElementById("ha-tipo-top").textContent = topTipo
    ? topTipo[0]
    : "—";
}
...
function renderTiposBar() {
  ...
  cont.innerHTML = Object.entries(freq)
    .sort((a, b) => b[1] - a[1])
    .map(([tipo, count]) => {
...
function filtrarTablaAdmin() {
  ...
  haFiltrados = haRegistros.filter((r) => {
    const matchQ =
      ...
      (r.identificador || "").toLowerCase().includes(q);
...
function renderTablaAdmin() {
  ...
  if (count)
    count.textContent = `${haFiltrados.length} registro${haFiltrados.length !== 1 ? "s" : ""}`;
  ...
  tbody.innerHTML = pagina
    .map((r, i) => {
      ...
      const entrada = fechaColombia(r.fecha_entrada).toLocaleTimeString(
        "es-CO",
        { hour: "2-digit", minute: "2-digit", timeZone: "America/Bogota" },
      );
      const salida = r.fecha_salida
        ? fechaColombia(r.fecha_salida).toLocaleString("es-CO", {
            dateStyle: "short",
            timeStyle: "short",
            timeZone: "America/Bogota",
          })
      ...
      const dur =
        r.duracion_min != null
          ...
          : r.estado === "activo"
            ? '<span style="color:#a5d6a7;font-size:11px;">En curso</span>'
            : "—";
      const badge =
        r.estado !== "activo"
...
function renderPaginAdmin(total, totalPags) {
  ...
  cont.appendChild(
    mk('<i class="bi bi-chevron-left"></i>', haPage - 1, haPage === 1, false),
  );
...
function exportarAdminExcel() {
  ...
  const rows = haRegistros.map((r) => {
    ...
    const tipo =
      tipoLabel[(r.tipo_vehiculo || "").toLowerCase()] ||
      r.tipo_vehiculo ||
      "—";
...
<table>
  ...
  <tr>
    ...
    <td style="background:${VERDE2};color:#ffffff;font-size:10pt;font-weight:700;
        ...
        text-align:center;letter-spacing:0.3pt;">${c}</td>`,
      )
      .join("")}
  </tr>
  <!-- Datos -->
