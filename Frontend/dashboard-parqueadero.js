  ...
  if (!vehiculos.length) {
    try {
      ...
      if (res.ok) {
        ...
        vehiculos = res.data.map((v) => ({
          ...v,
          tipo: v.tipo || mapTipo[v.id_tipo] || "",
        }));
      }
    } catch (e) {}
  }
...
async function loadCupos() {
  try {
    ...
    if (rol === "aprendiz") {
      ...
      if (cardBicis) {
        cardBicis.style.display = "";
        document.getElementById("num-bicis-a").textContent =
          d.lado_a.bicicletas ?? 0;
      }
      ...
      if (tituloB)
        tituloB.innerHTML =
          '<i class="bi bi-p-square"></i> Lado B <span style="font-size:11px;font-weight:400;opacity:.55;">(espacio abierto)</span>';
      ...
      if (cardCarros) {
        cardCarros.style.display = "";
        document.getElementById("num-carros-a").textContent =
          d.lado_a.carros ?? 0;
      }
      if (cardMotos) {
        cardMotos.style.display = "";
        document.getElementById("num-motos-a").textContent =
          d.lado_a.motos ?? 0;
      }
    ...
    // Solo visible para funcionarios/instructores — los aprendices no necesitan ver los cupos
    const ladoALabel = document.getElementById("lado-a-cupos-label");
    if (ladoALabel && cap > 0 && rol !== "aprendiz") {
...
function switchParkLado(lado, btn) {
  document.querySelectorAll(".dash-lado-tab").forEach((t) => {
    ...
    if (t.id === "park-tab-a" || t.id === "park-tab-b")
      t.classList.remove("active");
  });
  if (btn) btn.classList.add("active");
  document.getElementById("park-panel-A").style.display =
    lado === "A" ? "" : "none";
  document.getElementById("park-panel-B").style.display =
    lado === "B" ? "" : "none";
}

// ════════ HISTORIAL (sección completa) ════════
let histPage = 1;
let histTodosLosRegistros = []; // dashboard-perfil.js — Perfil, vehículos, utilidades e inicialización
("use strict");
...
async function loadHistorial(page = 1) {
  ...
  try {
    ...
    tbody.innerHTML = ultimos
      .map((r) => {
        const entrada = new Date(r.fecha_entrada).toLocaleString("es-CO", {
          dateStyle: "medium",
          timeStyle: "short",
          timeZone: "America/Bogota",
        });
        const salida = r.fecha_salida
          ? new Date(r.fecha_salida).toLocaleTimeString("es-CO", {
              hour: "2-digit",
              minute: "2-digit",
              timeZone: "America/Bogota",
            })
...
function calcularResumen(registros) {
  ...
  const promMin = completados.length
    ? Math.round(
        completados.reduce((s, r) => s + Number(r.duracion_min), 0) /
          completados.length,
      )
    : 0;
  ...
  if (elProm)
    elProm.textContent = promH > 0 ? `${promH}h ${promM}m` : `${promM}m`;
...
function aplicarFiltros() {
  ...
  if (ini)
    filtrados = filtrados.filter(
      (r) => new Date(r.fecha_entrada) >= new Date(ini),
    );
  if (fin)
    filtrados = filtrados.filter(
      (r) => new Date(r.fecha_entrada) <= new Date(fin + "T23:59:59"),
    );
  ...
  if (estado)
    filtrados = filtrados.filter((r) =>
      estado === "activo" ? r.estado !== "completado" : r.estado === estado,
    );
...
function renderTablaHistorial(registros) {
  ...
  if (label)
    label.textContent = `${registros.length} registro${registros.length !== 1 ? "s" : ""}`;
  ...
  tbody.innerHTML = pagina
    .map((r, i) => {
      ...
      const tipoIcon =
        tLow === "auto" || tLow === "carro" || tLow === "furgoneta"
          ? "bi-car-front-fill"
          : tLow === "motocicleta" || tLow === "moto"
            ? "bi-scooter"
            : "bi-bicycle";
  ...
  if (info)
    info.textContent = `Página ${histPage} de ${totalPags} · ${registros.length} registros`;
}
...
function renderPaginacion(total, totalPags) {
  ...
  for (let p = 1; p <= totalPags; p++) {
    if (
      ...
      p > 2 &&
      p < totalPags - 1 &&
      Math.abs(p - histPage) > 1
    ) {
      if (p === 3 || p === totalPags - 2) {
        const d = document.createElement("span");
        d.textContent = "…";
        d.style.cssText = "padding:0 4px;color:rgba(255,255,255,0.3);";
        cont.appendChild(d);
      }
      continue;
    }
...
function verDetalle(r) {
  ...
  body.innerHTML =
    ...
    campo(
      "bi-car-front-fill",
      "Vehículo",
      `${r.tipo_vehiculo} · ${r.identificador}`,
    ) +
...
function exportarHistorial() {
  if (!histTodosLosRegistros.length) {
...
async function loadPerfil() {
  try {
    ...
    // Tipo de ID: campo visual (solo lectura) + valor oculto para el backend
    const tipoLabels = {
      TI: "Tarjeta de Identidad",
      CC: "Cédula de Ciudadanía",
    };
    const tipoDisplay = document.getElementById("p-tipo-id-display");
    if (tipoDisplay)
      tipoDisplay.value =
        tipoLabels[data.data.tipo_id] || data.data.tipo_id || "";
    ...
    // Usar id_region de la BD; como fallback, el localStorage (guardado en saveProfile)
    const idRegion = data.data.id_region || Auth.getUser()?.id_region || null;
    const idCentro = data.data.id_centro || null;
    if (idRegion) {
      setValue("p-region", idRegion);
      await loadCentros(idRegion);
      if (idCentro) setValue("p-centro", idCentro);
    }
    ...
    if (data.data.foto_perfil && imgEl) {
      ...
      if (tbAv) {
        tbAv.style.backgroundImage = `url(${data.data.foto_perfil})`;
...
function onRolChange() {
  ...
  if (!rol) {
    ...
    if (badge) badge.textContent = "Sin rol asignado";
    return;
  }
  if (badge) {
...
async function saveProfile() {
  ...
  if (!nombre) {
    ...
    if (saveBtn) {
      ...
      saveBtn.innerHTML = '<i class="bi bi-check2-circle"></i> Guardar cambios';
    }
    return;
  }
  if (!tipo_id) {
    showToast("Selecciona tu tipo de identificación.", "error");
    if (saveBtn) {
      saveBtn.disabled = false;
      saveBtn.innerHTML = '<i class="bi bi-check2-circle"></i> Guardar cambios';
    }
  ...
  if (!num_id) {
    showToast("Ingresa tu número de identificación.", "error");
    if (saveBtn) {
      saveBtn.disabled = false;
      saveBtn.innerHTML = '<i class="bi bi-check2-circle"></i> Guardar cambios';
    }
  ...
  if (!centro) {
    showToast("Debes seleccionar tu centro de formación.", "error");
    if (saveBtn) {
      saveBtn.disabled = false;
      saveBtn.innerHTML = '<i class="bi bi-check2-circle"></i> Guardar cambios';
    }
  ...
  try {
    ...
    // Actualizar user en localStorage (incluir id_region para restaurar el select de región al recargar)
    const user = Auth.getUser();
    const region = document.getElementById("p-region").value;
    Auth.save({
...
async function changePassword() {
  const cpBtn =
    document.querySelector("#section-seguridad .btn-save") ||
    document.querySelector('[onclick*="changePassword"]');
  ...
  if (!actual || !nuevo || !confirm) {
    showToast("Completa los tres campos de contraseña.", "error");
    return;
  }
  if (nuevo.length < 8) {
  ...
  if (nuevo !== confirm) {
    if (hint) {
      ...
      hint.style.background = "rgba(239,83,80,.12)";
      hint.style.color = "#ef9a9a";
      hint.style.border = "1px solid rgba(239,83,80,.3)";
  ...
  try {
    const data = await apiPut("/usuarios/cambiar-password", {
...
async function loadVehiculos() {
  try {
    ...
    console.warn("loadVehiculos:", e);
  }
}

function getCurrentRol() {
  return document.getElementById("p-rol").value;
}
...
async function saveVehicle(tipo, svBtn) {
  ...
  if (svBtn) {
    svBtn.disabled = true;
    svBtn.innerHTML = '<i class="bi bi-hourglass-split"></i> Guardando...';
  }
  const rol = getCurrentRol();
  if (!rol) {
    showToast("Configura tu rol en el perfil primero.", "error");
    if (svBtn) {
      svBtn.disabled = false;
      svBtn.innerHTML = '<i class="bi bi-floppy"></i> Guardar vehículo';
    }
    return;
  }
  ...
  if (tipo === "bicicleta") {
    ...
    if (!modelo || !color) {
      showToast("Completa modelo y color.", "error");
      return;
    }
    ...
    if (!placa || !color) {
      showToast("Completa placa y color.", "error");
      return;
    }
...
function clearVehicleForm(tipo) {
  const ids =
    tipo === "bicicleta"
      ? ["bici-modelo", "bici-color", "bici-desc"]
      : [tipo + "-placa", tipo + "-color", tipo + "-desc"];
  ids.forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.value = "";
  });
  const p = document.getElementById(
    tipo === "bicicleta" ? "preview-bici" : "preview-" + tipo,
  );
  if (p) {
    p.src = "";
  ...
  const f = document.getElementById(
    tipo === "bicicleta" ? "foto-bici" : "foto-" + tipo,
  );
  if (f) f.value = "";
}
...
async function quitarFotoPerfil() {
  try {
    ...
    if (tbAv) {
      ...
      tbAv.textContent = tbAv.textContent || "U";
    }
...
function openDeleteModal(index) {
  ...
  const label = v.placa
    ? `${TIPO_LABELS[v.tipo]} · Placa ${v.placa}`
    : `${TIPO_LABELS[v.tipo]} · ${v.modelo || v.color}`;
...
async function confirmDelete() {
  ...
  const cdBtn =
    document.querySelector("#delete-modal .btn-save") ||
    document.querySelector('[onclick*="confirmDelete"]');
